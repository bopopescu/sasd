...
func checkAndRecoverDeadMain(analysisEntry inst.ReplicationAnalysis, candidateInstanceKey *inst.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (bool, *TopologyRecovery, error) {
	...
	if promotedReplica != nil {
		...
		if config.Config.ApplyMySQLPromotionAfterMainFailover {
			...
			AuditTopologyRecovery(topologyRecovery, fmt.Sprintf("- RecoverDeadMain: have applied read-only=0, RESET SLAVE ALL on promtoed main"))
...
func ForceMainFailover(clusterName string) (topologyRecovery *TopologyRecovery, err error) {
	...
	analysisEntry, err := forceAnalysisEntry(clusterName, inst.DeadMain, "force-main-failover", &clusterMain.Key)
	if err != nil {
		return nil, err
	}
...
func ForceMainTakeover(clusterName string, destination *inst.Instance) (topologyRecovery *TopologyRecovery, err error) {
	...
	analysisEntry, err := forceAnalysisEntry(clusterName, inst.DeadMain, "force-main-takeover", &clusterMain.Key)
	if err != nil {
		return nil, err
	}
...
func GracefulMainTakeover(clusterName string, designatedKey *inst.InstanceKey) (topologyRecovery *TopologyRecovery, promotedMainCoordinates *inst.BinlogCoordinates, err error) {
	...
	if len(clusterMainDirectReplicas) > 1 {
		return nil, nil, fmt.Errorf("GracefulMainTakeover: main %+v should only have one replica (making the takeover safe and simple), but has %+v. Aborting", clusterMain.Key, len(clusterMainDirectReplicas))
	}
	...
	log.Infof("Will demote %+v and promote %+v instead", clusterMain.Key, designatedInstance.Key)

	replicationUser, replicationPassword, replicationCredentialsError := inst.ReadReplicationCredentials(&designatedInstance.Key)

	analysisEntry, err := forceAnalysisEntry(clusterName, inst.DeadMain, "graceful-main-takeover", &clusterMain.Key)
	if err != nil {
		return nil, nil, err
	}
	...
	if err := executeProcesses(config.Config.PreGracefulTakeoverProcesses, "PreGracefulTakeoverProcesses", preGracefulTakeoverTopologyRecovery, true); err != nil {
		return nil, nil, fmt.Errorf("GracefulMainTakeover: failed running PreGracefulTakeoverProcesses: %+v", err)
	}

	if designatedInstance, err = inst.StopSubordinate(&designatedInstance.Key); err != nil {
		return nil, nil, err
	}
	log.Infof("Will set %+v as read_only", clusterMain.Key)
	if clusterMain, err = inst.SetReadOnly(&clusterMain.Key, true); err != nil {
		return nil, nil, err
	}

	log.Infof("Will advance %+v to main coordinates %+v", designatedInstance.Key, clusterMain.SelfBinlogCoordinates)
	...
	clusterMain, err = inst.ChangeMainTo(&clusterMain.Key, &designatedInstance.Key, promotedMainCoordinates, false, gitHint)
