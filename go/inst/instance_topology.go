...
func MultiMatchBelow(replicas [](*Instance), belowKey *InstanceKey, replicasAlreadyStopped bool, postponedFunctionsContainer *PostponedFunctionsContainer) ([](*Instance), *Instance, error, []error) {
	...
	if !replicasAlreadyStopped {
		...
		replicas = StopSubordinatesNicely(replicas, time.Duration(config.Config.InstanceBulkOperationsWaitTimeoutSeconds)*time.Second)
	}
	replicas = RemoveNilInstances(replicas)
			...
			func() {
				...
				for _, replica := range bucketReplicas {
					...
					go func() {
						...
						log.Debugf("MultiMatchBelow: Will match up %+v to previously matched main coordinates %+v", replica.Key, *bucketMatchedCoordinates)
						replicaMatchSuccess := false
						ExecuteOnTopology(func() {
							if _, err = ChangeMainTo(&replica.Key, &belowInstance.Key, bucketMatchedCoordinates, false, GTIDHintDeny); err == nil {
								StartSubordinate(&replica.Key)
...
func RegroupReplicasGTID(mainKey *InstanceKey, returnReplicaEvenOnFailureToRegroup bool, onCandidateReplicaChosen func(*Instance)) ([](*Instance), [](*Instance), [](*Instance), *Instance, error) {
...
func RegroupReplicas(mainKey *InstanceKey, returnReplicaEvenOnFailureToRegroup bool,
	...
	if allGTID {
		...
		unmovedReplicas, movedReplicas, cannotReplicateReplicas, candidateReplica, err := RegroupReplicasGTID(mainKey, returnReplicaEvenOnFailureToRegroup, onCandidateReplicaChosen)
